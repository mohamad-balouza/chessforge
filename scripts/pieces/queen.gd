class_name Queen
extends BasePiece
## Queen piece - combines rook and bishop movement

func _init() -> void:
	piece_type = PieceType.QUEEN
	piece_name = "Queen"
	piece_value = 9
	can_jump = false
	max_range = -1  # Unlimited


func _ready() -> void:
	super._ready()
	_setup_move_patterns()


func _setup_move_patterns() -> void:
	# Queen combines rook and bishop moves
	var queen_pattern := MovePattern.new()
	queen_pattern.directions = [
		# Rook directions
		Vector2i(1, 0),   # Right
		Vector2i(-1, 0),  # Left
		Vector2i(0, 1),   # Down
		Vector2i(0, -1),  # Up
		# Bishop directions
		Vector2i(1, 1),   # Down-right
		Vector2i(1, -1),  # Up-right
		Vector2i(-1, 1),  # Down-left
		Vector2i(-1, -1)  # Up-left
	]
	queen_pattern.max_range = -1  # Unlimited sliding
	queen_pattern.mirror_for_black = false
	
	move_patterns = [queen_pattern]


## Override to calculate legal moves with sliding behavior
func get_legal_moves() -> Array[Vector2i]:
	var moves: Array[Vector2i] = []
	
	if not board:
		return moves
	
	var directions := [
		Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1),
		Vector2i(1, 1), Vector2i(1, -1), Vector2i(-1, 1), Vector2i(-1, -1)
	]
	
	for dir in directions:
		moves.append_array(_get_sliding_moves(dir))
	
	return _apply_movement_modifiers(moves)


func _get_sliding_moves(direction: Vector2i) -> Array[Vector2i]:
	var moves: Array[Vector2i] = []
	var current := board_position + direction
	
	while board.is_valid_square(current):
		var piece_at_target: BasePiece = board.get_piece_at(current)
		
		if piece_at_target:
			# Can capture enemy pieces
			if piece_at_target.piece_color != piece_color:
				moves.append(current)
			break  # Stop at any piece
		else:
			moves.append(current)
		
		current += direction
	
	return moves


func get_attack_squares() -> Array[Vector2i]:
	return get_legal_moves()

